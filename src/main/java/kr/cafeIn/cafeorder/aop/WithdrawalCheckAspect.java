...
/**AOP 해당 어노테이션이 있다면 로그인을 체크해야 한다 (공통로직)
 * 로그인 검증 AOP.
 *
 * @since 1.0.0
 */
...
public class WithdrawalCheckAspect {
    ...
    /**
     * 회원가입시 탈퇴 후 재가입 회원인지 확인 후 재가입 회원일 경우 register 메소드를 호출 한다.
     * register 호출 하여 탈퇴후 3일이 지난 회원인 경우는  withdrawAt 값을 null 로  업데이트 진행
     * {@link WithdrawalCheck}가 붙어있는 메소드에 회원가입 검증 AOP 적용.
     *
     * @since 1.0.0
     */
